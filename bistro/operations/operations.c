...
char *parse_multiplication(char *base, char *operators, char *expr1, char *expr2)
{
  ...
  if ((flags = malloc(sizeof(*flags) * 3)) == 0)
    return (NULL);
  flags = parse_operation(base, operators , &expr1, &expr2);
  if ((flags[0] == 0) && (flags[1] == 0))// cas 3 * 3
...
char	*parse_addition(char *base, char *operators,char *expr1, char *expr2)
{
  ...
  if ((my_strlen(expr1) == 1) && (expr1[0] == base[0]))
    if ((my_strlen(expr2) == 1) && (expr2[0] == base[0]))
    return (addition(base, expr1, expr2));
  if ((flags[0] == 0) && (flags[1] == 0))// cas 4 + 4
    return (addition(base, expr1, expr2));
  if ((flags[0] == 1) && (flags[1] == 1))// cas -4 -4
    return (ajoute_moins(operators, addition(base, expr1, expr2)));
  if (flags[2] == 0) // expr 1 > expr2 (sans compter les moins)
    {
      if (flags[0] == 1)
	return (soustraction(base, operators, expr2, expr1));// cas -5 + 4
      return (soustraction(base, operators , expr1, expr2));// cas 5 + - 4
    }
  if (flags[2] == 1)// expr2 > expr1
    {
      if (flags[0] == 1)// -4 +5
	return (soustraction(base, operators,expr2, expr1));
      return(soustraction(base, operators,expr1, expr2));// 4 + -5
    }
}

/*
**Retourne la negation d'un nombre 
*/
...
/*
** Parse operation, check kel est le nombre superieur, et sils sont negatifs
** flag[0] = 1 si expr1 est negatif
*/
...
/*
**renvoi le nombre de zeros a retirer au debut dune expr
*/
